<?
	...
	chdir(APK_DIST);
	$result = my_exec('..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'dist' . DIRECTORY_SEPARATOR . 'lib' . DIRECTORY_SEPARATOR . 'zipalign -f 4 ' . escapeshellarg($apk_name) . ' ' . escapeshellarg($apk_name . '-unsigned-debug.apk'));
	if ($result['return'] != 0)
	{
		print_r($result);
		die;
	...
	exec('explorer /e,/select,' . escapeshellarg(APK_DIST . $apk_name . '-unsigned-debug.apk'));